...
────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────' \
    ...
    --bind 'd:execute-silent(for cluster in {+1}; do tmux send-keys "/usr/local/bin/ocpdestroycluster $cluster" C-m; sleep 2; done)+abort' \
    --bind 's:execute-silent(for cluster in {+1}; do tmux send-keys "/usr/local/bin/ocpstartcluster $cluster" C-m; sleep 2; done)+abort' \
    --bind 'S:execute-silent(for cluster in {+1}; do tmux send-keys "/usr/local/bin/ocpstopcluster $cluster" C-m; sleep 2; done)+abort' \
